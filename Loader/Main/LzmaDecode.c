...
unsigned long LZMA_Decompress( void *pDst, unsigned long *pulDstLen, void *pSrc, unsigned long ulSrcLen )
{
  ...
  unsigned int * test = pSrc, temp;
   ...
   serial_outstr("LZMA_Decompress start here.........\n");
/*
...
*/
  ...
  {
#ifdef LZMA_DEBUG
    serial_outstr("LZMA_Decompress() : Input mem len is zero\n");
#endif
    return 1;
  }
  ...
  {
#ifdef LZMA_DEBUG
    serial_outstr("LZMA_Decompress() : stream version is not supported\n");
#endif
    return 1;
  }
  ...
  for (ii = 0; ii < 4; ii++)
  {
    ...
    {
#ifdef LZMA_DEBUG
      serial_outstr("LZMA_Decompress() :  too long file\n");
#endif
      return 1;
    }
  }
  ...
  {
#ifdef LZMA_DEBUG
    serial_outstr("LZMA_Decompress() : Properties error\n");
#endif
    return 1;
  }
  for (pb = 0; prop0 >= (9 * 5);
  ...
  {
#ifdef LZMA_DEBUG
    serial_outstr("LZMA_Decompress() : outSize >= *pulDstLen\n");
#endif
    return 1;
  }


  //lzmaInternalData = K_malloc(lzmaInternalSize);
  lzmaInternalData = (void *)LZMA_INTERNAL_BUFF;

  //if (outStream == 0 || lzmaInternalData == 0)
  if (lzmaInternalData == 0)
  {
#ifdef LZMA_DEBUG
    serial_outstr("LZMA_Decompress() : outStream == 0 || lzmaInternalData == 0\n");
#endif
    return 1;
  }
  ...
  #ifdef _LZMA_OUT_READ
  {
    ...
    {
	#ifdef LZMA_DEBUG
	serial_outstr("LZMA_Decompress() : dictionary == 0\n");
	#endif
      return 1;
    }
    ...
    for (nowPos = 0; nowPos < outSize;)
    {
      ...
      {
        #ifdef LZMA_DEBUG
	serial_outstr("LZMA_Decompress() : res != 0\n");
	#endif
        return 1;
      }
  ...
  *pulDstLen = outSize;
#ifdef LZMA_DEBUG
  serial_outstr("LZMA_Decompress() : good!\n");
#endif
  return 0;
