...
UINT8_T Giga_SPINor_ReadReg(Giga_Register reg)
{
	UINT_T dummy;
	...
	switch (reg)
	{
		case Giga_Status_Register:
			command = GIGA_SPI_CMD_READ_STATUS;
			break;

		case Giga_Status_Register1:
			command = GIGA_SPI_CMD_READ_STATUS1;
			break;
...
void Giga_SPINor_WriteStatus(unsigned char status, unsigned char status1)
{
	unsigned int temp;
	...
	temp = GIGA_SPI_CMD_WRITE_STATUS_REG << 16;
	temp |= status << 8;
	temp |= status1;
...
void Giga_SPINor_Protect_Blocks(void)
{
...
void Giga_SPINor_UnProtectBlocks(void)
{
	UINT_T status_value, status1_value;
	...
	if (((status_value & (GIGA_STATUS_BP0 | GIGA_STATUS_BP1 | GIGA_STATUS_BP2)) == 0) ||
		((status_value & (GIGA_STATUS_BP0 | GIGA_STATUS_BP1 | GIGA_STATUS_BP2)) == (GIGA_STATUS_BP0 | GIGA_STATUS_BP1 | GIGA_STATUS_BP2)))
	{
	...
	status_value &= ~(GIGA_STATUS_BP0 | GIGA_STATUS_BP1 | GIGA_STATUS_BP2);
...
void Giga_Enable4BytesMode(void)
{
	...
	serial_outstr("Giga_Enable4BytesMode\n");
	...
	//load the command
	reg_write(SSP_DR, GIGA_SPI_CMD_ENABLE_4BYTE_MODE);
...
void Giga_Disable4BytesMode(void)
{
	...
	//load the command
	reg_write(SSP_DR, GIGA_SPI_CMD_DISABLE_4BYTE_MODE);
...
UINT_T Giga_EraseSector(UINT_T secAddr)
{
	...
	temp[0] = GIGA_SPI_CMD_SECTOR_ERASE;
